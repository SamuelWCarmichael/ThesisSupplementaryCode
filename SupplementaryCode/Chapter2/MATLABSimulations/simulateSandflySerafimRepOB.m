% This version of the retroleptomonad model is special - designed to
% replicate the experiment of Serafim et al. and thus has some specific
% modifications.
function F = simulateSandflySerafimRepOB()
% No input currently, since this function shouldn't require any.

% Loading some important global variables.
global meanLS meanBiteFreq retroDevT biteScaleParam

lifespan = 20.0; % Choosing a lifespan for the fly from an exponential random variable.

firstBiteT = 0.01; % Choose the time for the first bite, exponential dist. in this version.
...
if firstNectoPeakT > lifespan
    F.popArray = [0,0,0,0]; % Population values over time.
...
if firstBiteT <= 0.001
    evtA = " "+string(firstBiteT)+" Bite"+" NA";
...
ICVec = [0;0;0;0]; % No nectomonads, leptomonads, metacyclics or retroleptomonads to start.
...
while elapsedTime < lifespan
    ...
    % An if statement here is needed to avoid problems arising from
    % coinciding events, which would otherwise break linspace.
    if elapsedTime ~= endTime
        % We now have the time we need to run ode45 for. Some ode45 prep:
        tSpan = linspace(elapsedTime, endTime, 1000); % A suitable time vector.
        ...
        % This model has two different modes - normal and dedifferentiation.
        if infState == "Dediff"
            % This indicates that an infected fly recently bit again.
            [t, pVec] = ode45(@dediffModel, tSpan, ICVec);
        else
            % Either the bite was a while ago or the fly hasn't bitten.
            [t, pVec] = ode45(@retroModel, tSpan, ICVec);
        end
    ...
    elseif evtType == "Bite" && infState == "Naive"
        ...
        % the vector.
        numBites = numBites + 1;
        numAmast = biteCalculator(numBites); % Calculate if/number of parasites uptaken.
        if numAmast > 0
        ...
        nextBiteT = elapsedTime + 50.0; % forces the next bite to occur after fly death.
    ...
    elseif evtType == "Bite" && infState == "Infected"
        ...
        nextBiteT = elapsedTime + 50.0; % Generating next bite time.
        if nextBiteT < lifespan
            % If the fly will still be alive when the bite occurs:
            if nextBiteT <= 0.001
                newBiteEvt = " "+string(nextBiteT)+" Bite"+" NA";
        ...
        % Next, we work out how many parasites are transferred to the
        % host (if any).
        ...
        % Finally, we cause the system to enter dedifferentiation mode.
        infState = "Dediff";
        dediffEndT = elapsedTime + retroDevT;
        if dediffEndT < lifespan
            % Otherwise the fly dies before this phase ends.
            if dediffEndT < 10 && lifespan >= 100
                newDediffEndEvt = string(0)+string(0)+string(dediffEndT)+" DediffEnd"+" NA";
            elseif (dediffEndT < 10 && lifespan < 100) || (dediffEndT >= 10 && dediffEndT < 100 && lifespan >= 100)
                newDediffEndEvt = string(0)+string(dediffEndT)+" DediffEnd"+" NA";
            else
                newDediffEndEvt = string(dediffEndT)+" DediffEnd"+" NA";
            end
            evtArray = [evtArray newDediffEndEvt];
        end
    elseif evtType == "Bite" && infState == "Dediff"
        % This is very similar to the other bites, but is an added
        % complication. The dediff period must be extended, which is a
        % pain!
        numBites = numBites + 1;
        numAmast = biteCalculator(numBites); % Calculate if/number of parasites uptaken.
        if numAmast > 0
        ...
        % Finally, we update the end point of the dedifferentiation phase.
        % Either we change the time, or replace it with a Death event.
        updDediffEndT = elapsedTime + retroDevT;
        if updDediffEndT < lifespan
            % The dediff ends before fly death.
            if updDediffEndT < 10 && lifespan >= 100
                updDediffEndEvt = string(0)+string(0)+string(updDediffEndT)+" DediffEnd"+" NA";
            elseif (updDediffEndT < 10 && lifespan < 100) || (updDediffEndT >= 10 && updDediffEndT < 100 && lifespan >= 100)
                updDediffEndEvt = string(0)+string(updDediffEndT)+" DediffEnd"+" NA";
            else
                updDediffEndEvt = string(updDediffEndT)+" DediffEnd"+" NA";
            end
            for i = 1:length(evtArray)
                if evtArray(i) == newDediffEndEvt
                    evtArray(i) = updDediffEndEvt;
                end
            end
            newDediffEndEvt = updDediffEndEvt;
        elseif updDediffEndT > lifespan && dediffEndT < lifespan
            % The fly dies before this happens. We replace it with a death
            % event.
            ...
            % Both end points are/were out of the lifespan. Do nothing.
        end
    elseif evtType == "DediffEnd"
        % This only changes the system state back to infected.
        infState = "Infected";
...
function F = biteCalculator(numBites)
