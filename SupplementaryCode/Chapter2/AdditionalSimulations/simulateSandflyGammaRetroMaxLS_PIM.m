% This version of the retroleptomonad model uses gamma distribution of mean
% 6 and scalar variable of 0.5 instead of an exponential distribution (for
% mean bite frequency).

function F = simulateSandflyGammaRetroMaxLS_PIM()
% No input currently, since this function shouldn't require any.

% Loading some important global variables.
global meanLS meanBiteFreq retroDevT biteScaleParam lifespanReduction maxLifespan
...
if firstNectoPeakT > lifespan
    ...
    F.lifespan = lifespan;
    F.pTr = [0]; % This is a vector of the number of parasites transferred at each bite - 0 here to not mess up my collector script!
    return
end
...
pTr = []; % Empty vector which will contain the number of parasites transferred each bite.
