...
function [F,G,H] = discr0commstructthreshsimrep(rType,infDur,symT, prop, reps)
...
for rep = 1:reps
    ...
    disp(rep)
    extinct = 0; % A nice little indicator of whether the simulation went extinct.
    extTime = 0; % Records when the sim went extinct, another useful bit of info.
    ...
    diagnostics(rep,:) = [rep,prop,extinct,extTime,iprop,deltf,mean(nConn),var(nConn),mean(nConn.^3)];
